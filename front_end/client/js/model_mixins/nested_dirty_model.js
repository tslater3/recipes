export default /* @ngInject */ function(restmod) {
  ...
  function isPlainObject(_val) {
    return angular.isObject(_val) && !angular.isArray(_val) && !_.isDate(_val);
  }
  ...
  return restmod.mixin(function() {
    this.on('after-feed', function() {
        ...
        .on('after-init', function() {
          if(!this.hasOwnProperty('$cmStatus')) {
            this.$cmStatus = copyOriginalData(this);
          }
        })
        .on('after-reset-attributes', function(){
          this.$takeAttributesSnapshot();
        })
        .define('$attributeHasChanged', function(attribute_name) {
          let self = this,
              value = self.$cmStatus[attribute_name],
              is_blank_string = (_.isString(value) && !value.length);
          //if we set init to a blank string, i.e. someattr: { init: ''},
          //return false if it did not have a value set previously
          return (!is_blank_string && self.$isDirty() && self.$dirty(attribute_name));
        })
        /**
         * @method $dirty
        ...
        .define('$isDirty', function() {
          return !!this.$dirty().length;
        })
        .define('$isClean', function() {
          return !this.$isDirty();
        })
        .define('$takeAttributesSnapshot', function() {
          this.$cmStatus = copyOriginalData(this);
        })
        /**
         * @method $restore
